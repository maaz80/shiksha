...
import { ChevronDown, Grip, Menu, Search, X, LogOut, Phone as FaPhoneAlt, Mail as FaEnvelope } from 'lucide-react';
...
const SiFacebook = ({ size = 14, className = "" }) => (
...
export default function Navbar({ isModal, setIsModal, setIsLogin, isLogin, onCoursesMouseEnter, onCoursesMouseLeave }) {
     ...
     return (
          <header className="w-full fixed top-0 bg-primary-bg z-99999 font-sans">
               ...
               {/* Top Bar */}
               {!isDashboardPage && (
                    <div className="w-full bg-primary text-white py-1.5 md:py-2 text-[11px] md:text-[13px] font-semibold border-b border-zinc-950/10 relative z-50 open-sans">
                         <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 flex-col md:flex-row px-3 md:px-0 pb-0.5 md:pb-0">
                              <div className="flex items-center gap-4 md:gap-6">
                                   <a href="tel:+919311500424" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                                        <FaPhoneAlt size={12} className="md:w-3.5 md:h-3.5" aria-hidden="true" />
                                        <span>+91 9311500424</span>
                                   </a>
                                   <ObfuscatedEmail email="info@shikshadesign.com" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                                        <FaEnvelope size={12} className="md:w-3.5 md:h-3.5" aria-hidden="true" />
                                        <span>info@shikshadesign.com</span>
                                   </ObfuscatedEmail>
                              </div>
               ...
               <div className="mx-auto w-full max-w-350 h-15 md:h-16 lg:h-20 px-4 sm:px-6 lg:px-13 flex items-center justify-between gap-1 md:gap-4">
                    {!isDashboardPage && (
                         <div className='2xl:hidden text-secondary cursor-pointer'>
                              ...
                              <div
                                   ...
                                   className={`fixed inset-0 bg-black/40 z-50 transition-opacity duration-300 ${isMenu ? "opacity-100 visible" : "opacity-0 invisible"
                                        }`}
                              />

                              <div
                                   className={`fixed top-0 left-0 h-screen w-[85%] max-w-90 bg-white z-50 flex flex-col justify-between transition-all duration-300 ease-in-out ${isMenu ? "translate-x-0" : "-translate-x-full"
                                        }`}
                              >
                                   {/* Top */}
                                   <div className="p-5">
                                             ...
                                             {((moreItems?.dropdown_items && moreItems.dropdown_items.length > 0) ? moreItems.dropdown_items : fallbackMoreItems.dropdown_items).map((cat, catIdx) => (
                                                  <div key={catIdx} className="flex flex-col gap-2 pl-2 text-left">
                                                       ...
                                                       {cat.items && cat.items.map((subItem, subIdx) => {
                                                            const targetLink = (subItem.link === '/category/blogs' || subItem.link === '/category/blog') ? '/blog' : subItem.link;
                                                            const isHashOrEmpty = !targetLink || targetLink === "#";
                         ...
                         {!isDashboardPage && (
                              <div className="flex items-center flex-1 justify-center gap-3 lg:gap-5 min-w-0">
                                   ...
                                   <div ref={searchContainerRef} className="flex-1 max-w-105 min-w-45 relative hidden 2xl:block">
                                        <form onSubmit={handleSearchSubmit} className="relative w-[288px] group">
                                             <input
                                                  type="text"
                                                  value={searchTerm}
                                                  onChange={(e) => setSearchTerm(e.target.value)}
                                                  placeholder={searchPlaceholder}
                                                  className="w-70 h-11 pl-11 pr-4 rounded-md bg-white outline-none text-base text-neutral border-2 border-transparent group-hover:border-primary/70 focus:border-primary/70 transition-all duration-300 placeholder:text-base group-hover:placeholder:text-neutral-700 placeholder:transition-all placeholder:duration-300"
                                             />
                                             <button type="submit" aria-label="Submit search" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-primary transition-colors cursor-pointer border-0 bg-transparent p-0 group-hover:text-primary">
                                                  <Search className="h-4 w-4" />
                                             </button>
                                        </form>
                                        {searchTerm.length >= 2 && (
                                             <div className="absolute top-full mt-2 left-0 w-80 bg-white border border-zinc-200 rounded-md shadow-2xl overflow-y-auto max-h-80 z-50">
                                                  {loadingSearch ? (
                    ...
                    {/* Right section */}
                    <div className={`items-center gap-4 lg:gap-2 shrink-0 text-sm text-gray-700 ${isDashboardPage ? 'flex' : 'hidden 2xl:flex'}`}>
                         {!isDashboardPage && (
                              <div
                                   className="relative group self-start hover:mt-4 transition-all duration-300 ease-in-out"
                                   onMouseEnter={() => setIsMore(true)}
                                   onMouseLeave={() => setIsMore(false)}
                              >
                                   <button className="hidden sm:flex h-12 group-hover:h-14.5 w-27.5 items-center justify-center rounded-t-md text-secondary text-[14px] transition-all duration-300 ease-in-out cursor-pointer group-hover:bg-white">
                                   ...
                                   <div
                                        className={`absolute top-15 right-0 bg-white border border-gray-100 p-6 rounded-b-lg opacity-0 invisible -translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 z-50 shadow-lg ${colsCount === 1 ? "w-60" : colsCount === 2 ? "w-md" : colsCount === 3 ? "w-160" : "w-268"
                                             }`}
                                   >
                                        <div className={`grid gap-6 text-left ${colsCount === 1 ? "grid-cols-1" : colsCount === 2 ? "grid-cols-2" : colsCount === 3 ? "grid-cols-3" : "grid-cols-4"
                                             }`}>
                         ...
                         <button onClick={handleLoginClick} className={`h-12 w-27.5 flex items-center justify-center rounded-md border text-[16px] transition-all duration-300 ease-in-out cursor-pointer ${isLoggedIn
               ...
               {!isDashboardPage && isSearchOpen && (
                    <div ref={mobileSearchRef} className="absolute top-full left-0 right-0 bg-white border-b border-gray-200 px-4 py-3 flex flex-col gap-2 2xl:hidden shadow-md z-50">
                         <div className="relative">
                              <form onSubmit={handleSearchSubmit} className="relative">
                                   <input
                                        ...
                                        placeholder={searchPlaceholder}
                                        autoFocus
                                        className="w-full h-10 border border-gray-300 rounded-md bg-white pl-10 pr-10 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                                   />
                                   <button type="submit" aria-label="Submit search" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-primary transition-colors cursor-pointer border-0 bg-transparent p-0">
                                        <Search className="h-4 w-4" />
                                   </button>
                                   {searchTerm && (
